...
class Strategy(BaseStrategy):
    """Nframe 选股回测策略 v1.0.1 — 策略净值曲线择时版。

    在 v1.0 基础上新增"方案A: 策略净值曲线择时"：
    - 逐日跟踪策略自身的模拟净值曲线
    - 净值 > N日均线 → 满仓运行（max_holdings 只）
    - 净值 < N日均线 → 降仓运行（timing_reduced_holdings 只）
    - 净值连续 K 日站上均线才恢复满仓（防止频繁切换）
    - 降仓时按当前收益最差的持仓优先卖出

    原理：这类微观结构模式策略的有效/失效期具有自相关性——
    一段时间内策略亏钱意味着市场微结构环境暂时不利于此模式，
    用策略自身净值作为最精准的"环境传感器"。
    """

    name = "Nframe选股回测策略v1.0.1净值择时"
    description = (
        "基于放量缩量回调模式选股（5日/4日），"
        "叠加策略净值曲线择时，净值低于均线自动降仓。"
    )
    ...
    def __init__(
        ...
        timing_nav_ma: int = 20,
        timing_reduced_holdings: int = 2,
        timing_confirm_days: int = 2,
    ) -> None:
        ...
        self.timing_nav_ma = int(timing_nav_ma)
        self.timing_reduced_holdings = int(timing_reduced_holdings)
        self.timing_confirm_days = int(timing_confirm_days)

    def get_config_schema(self) -> dict:
        return {
            "title": "Nframe v1.0.1 净值择时参数",
            "fields": [
                {"name": "max_holdings", "label": "满仓最大持股数", "type": "number", "min": 1, "max": 10, "step": 1, "default": self.max_holdings},
                {"name": "timing_nav_ma", "label": "择时-净值均线周期", "type": "number", "min": 5, "max": 60, "step": 1, "default": self.timing_nav_ma},
                {"name": "timing_reduced_holdings", "label": "择时-降仓持股数", "type": "number", "min": 0, "max": 5, "step": 1, "default": self.timing_reduced_holdings},
                {"name": "timing_confirm_days", "label": "择时-恢复满仓确认天数", "type": "number", "min": 1, "max": 10, "step": 1, "default": self.timing_confirm_days},
    ...
    def generate_portfolio_weights(
        ...
        above_ma_streak = 0
        timing_ma_period = self.timing_nav_ma
        timing_regime = "满仓"
        ...
        for day_idx, trade_date in enumerate(trade_index):
            ...
            # ══════ 0. 计算模拟净值 & 择时信号 ══════
            daily_ret = 0.0
            for ts_code_h, info_h in holdings.items():
                if info_h.get("days_held", 0) < 1:
                    continue
                sc_h = stock_cache.get(ts_code_h)
                if sc_h is None:
                    continue
                ri_h = sc_h["date_idx"].get(td_np)
                if ri_h is None or ri_h < 1:
                    continue
                prev_c = sc_h["close"][ri_h - 1]
                cur_c = sc_h["close"][ri_h]
                if prev_c > 0 and not np.isnan(prev_c) and not np.isnan(cur_c):
                    daily_ret += (cur_c / prev_c - 1.0) * info_h["weight"]

            nav *= (1.0 + daily_ret)
            nav_history.append(nav)

            if len(nav_history) >= timing_ma_period:
                nav_ma = np.mean(nav_history[-timing_ma_period:])
                if nav > nav_ma:
                    above_ma_streak += 1
                else:
                    above_ma_streak = 0

                if above_ma_streak >= self.timing_confirm_days:
                    effective_max = self.max_holdings
                    timing_regime = "满仓"
                else:
                    effective_max = self.timing_reduced_holdings
                    timing_regime = "降仓"
            else:
                effective_max = self.max_holdings
                timing_regime = "满仓"
            ...
            # ══════ 1.5 择时强制减仓 ══════
            if len(holdings) > effective_max:
                perf = []
                for ts_code, info in holdings.items():
                    sc = stock_cache.get(ts_code)
                    ri = sc["date_idx"].get(td_np) if sc else None
                    if sc is not None and ri is not None and info["buy_price"] > 0:
                        ret = sc["close"][ri] / info["buy_price"] - 1.0
                    else:
                        ret = -999.0
                    perf.append((ts_code, ret))
                perf.sort(key=lambda x: x[1])

                excess = len(holdings) - effective_max
                for ts_code, _ in perf[:excess]:
                    sig = f"{ts_code}|{trade_date.strftime('%Y-%m-%d')}"
                    sell_reasons[sig] = f"择时减仓-净值低于{timing_ma_period}日均线"
                    holdings.pop(ts_code)
                    self._cleanup_chips(ts_code)
                    sold_today.add(ts_code)
                    rebalance_count += 1
