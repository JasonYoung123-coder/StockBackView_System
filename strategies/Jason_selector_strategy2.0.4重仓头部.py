...
class Strategy(BaseStrategy):
    ...
    position_top_rank1 = 0.3
    max_boosted_stocks = 2
    ...
    def generate_portfolio_weights(
        ...
        for idx, trade_date in enumerate(trade_index, start=1):
            ...
            # ── 买入判断 ──
            if oamv_regime != "死叉" and self._has_position_capacity(holdings):
            ...
            if oamv_regime != "死叉":
                if not self._has_position_capacity(holdings):
            ...
            for ts_code in holdings:
                weights.loc[trade_date, ts_code] = holdings[ts_code].get("weight", self.position_per_stock)
    ...
    def _fill_positions(
        ...
        added = 0
        running_weight = self._total_position_weight(holdings)
        running_boosted = self._count_boosted_holdings(holdings)
        ...
        for row in candidates.itertuples():
            ...
            can_boost = (
                float(row.score) >= self.buy_score_threshold
                and row.Index < self.max_boosted_stocks
                and running_boosted < self.max_boosted_stocks
            )
            weight = self.position_top_rank1 if can_boost else self.position_per_stock

            if running_weight + weight > 1.0 + 1e-9:
                if weight > self.position_per_stock:
            ...
            if weight >= self.position_top_rank1:
                running_boosted += 1
            added += 1

            if running_weight + self.position_per_stock > 1.0 + 1e-9:
                break
        return added
    ...
    def _try_replace_holdings(
        ...
        for old_code, _ in replaceable:
            ...
            old_weight = holdings[old_code].get("weight", self.position_per_stock)
            boosted_after_sell = self._count_boosted_holdings(holdings) - (
                1 if old_weight >= self.position_top_rank1 else 0
            )
            can_boost = (
                float(new_row.score) >= self.buy_score_threshold
                and new_row.Index < self.max_boosted_stocks
                and boosted_after_sell < self.max_boosted_stocks
            )
            weight = self.position_top_rank1 if can_boost else self.position_per_stock

            current_total = self._total_position_weight(holdings)
            if current_total - old_weight + weight > 1.0 + 1e-9:
                if weight > self.position_per_stock:
                    weight = self.position_per_stock
                if current_total - old_weight + weight > 1.0 + 1e-9:
                    continue
    ...
    def _replace_losing_non_top_score_holdings(
        ...
        for candidate in top_score_candidates.itertuples():
            ...
            can_boost = (
                float(candidate.score) >= self.buy_score_threshold
                and candidate.Index < self.max_boosted_stocks
                and boosted_after_sell < self.max_boosted_stocks
            )
            weight = self.position_top_rank1 if can_boost else self.position_per_stock
    ...
    def _total_position_weight(self, holdings: dict[str, dict]) -> float:
        return sum(h.get("weight", self.position_per_stock) for h in holdings.values())

    def _count_boosted_holdings(self, holdings: dict[str, dict]) -> int:
        return sum(
            1 for h in holdings.values()
            if h.get("weight", self.position_per_stock) >= self.position_top_rank1
        )

    def _has_position_capacity(self, holdings: dict[str, dict]) -> bool:
        return self._total_position_weight(holdings) + self.position_per_stock <= 1.0 + 1e-9
