...
class Strategy(BaseStrategy):
    ...
    BEAR_STOCK_CODE = "601288.SH"
    BEAR_STOCK_NAME = "农业银行"
    BEAR_POSITION_RATIO = 0.30
    BEAR_STOCK_POSITION = 0.70
    BEAR_STOCK_KDJ_J_THRESHOLD = 13.0
    BEAR_STOCK_INITIAL_PROFIT_TARGET = 0.05
    BEAR_STOCK_REBUY_PROFIT_TARGET = 0.03
    ...
    def generate_portfolio_weights(
        ...
        for idx, trade_date in enumerate(trade_index, start=1):
            ...
            if (
                ...
                if (
                    ...
                    self._add_warning(
                        ...
                        "检测到手动卖出：策略模拟仍持有农业银行但QMT已无此持仓，已自动释放仓位",
                    )
            ...
            if self._bear_stock_holding is not None:
                bear_closed = self._check_bear_stock_stop_loss(trade_date, sell_reasons, sell_prices)
                if not bear_closed:
                    self._check_bear_stock_profit_sell(trade_date, sell_reasons, sell_prices)
            ...
            if market_regime == "熊市":
                ...
                if self._bear_stock_holding and self._bear_stock_holding["weight"] > self.BEAR_STOCK_MIN_WEIGHT:
                    regular_total = len(holdings) * self.position_per_stock
                    bear_cap = max(0.0, 1.0 - regular_total)
                    if bear_cap < self.BEAR_STOCK_MIN_WEIGHT:
                        key = f"{self.BEAR_STOCK_CODE}|{trade_date.strftime('%Y-%m-%d')}"
                        sell_reasons[key] = "牛市调仓-仓位已满卖出农业银行"
                        bp = self._get_bear_stock_price(trade_date)
                        if bp is not None:
                            sell_prices[key] = bp
                        self._bear_stock_holding = None
                    else:
                        if bear_cap < self._bear_stock_holding["weight"] - 0.001:
                            key = f"{self.BEAR_STOCK_CODE}|{trade_date.strftime('%Y-%m-%d')}"
                            sell_reasons[key] = "牛市调仓-减仓农业银行腾出仓位"
                            bp = self._get_bear_stock_price(trade_date)
                            if bp is not None:
                                sell_prices[key] = bp
                            self._bear_stock_holding["weight"] = bear_cap
                        weights.loc[trade_date, self.BEAR_STOCK_CODE] = self._bear_stock_holding["weight"]
    ...
    def _fetch_bear_stock_data(self, trade_index: pd.DatetimeIndex) -> None:
        """获取农业银行技术指标数据（仅用于信号判断，不用于成交定价）。"""
        start_str = (trade_index.min() - pd.Timedelta(days=30)).strftime("%Y%m%d")
        end_str = trade_index.max().strftime("%Y%m%d")

        stk_data = self._tushare.get_stk_factor_pro(
            self.BEAR_STOCK_CODE, start_str, end_str,
            fields="ts_code,trade_date,close,ma_bfq_30,kdj_bfq",
        )
        ...
        for col in ("close", "ma_bfq_30", "kdj_bfq"):
            if col in stk_data.columns:
                stk_data[col] = pd.to_numeric(stk_data[col], errors="coerce")
        stk_data["trade_date"] = pd.to_datetime(stk_data["trade_date"])
        stk_data = stk_data.drop_duplicates(subset=["trade_date"], keep="last").sort_values("trade_date")

        for _, row in stk_data.iterrows():
            td = pd.Timestamp(row["trade_date"]).normalize()
            self._bear_stock_factor_lookup[td] = {
                "close": row.get("close"),
                "ma_bfq_30": row.get("ma_bfq_30"),
                "kdj_bfq": row.get("kdj_bfq"),
            }
    ...
    def _handle_bear_to_bull_transition(
        ...
        """熊转牛：将收益率大于0且最高的股票补仓至正常仓位，减仓农业银行腾出空间。"""
        if not holdings:
            return
        ...
        total_regular_needed = len(holdings) * self.position_per_stock
        bear_cap = max(0.0, 1.0 - total_regular_needed)

        if (
            self._bear_stock_holding
            and self._bear_stock_holding["weight"] > self.BEAR_STOCK_MIN_WEIGHT
            and bear_cap < self._bear_stock_holding["weight"] - 0.001
        ):
            key = f"{self.BEAR_STOCK_CODE}|{trade_date.strftime('%Y-%m-%d')}"
            bp = self._get_bear_stock_price(trade_date)
            best_name = holdings[best_code].get("name", best_code)
            if bear_cap < self.BEAR_STOCK_MIN_WEIGHT:
                sell_reasons[key] = f"熊转牛-卖出农业银行为{best_name}补仓"
                if bp is not None:
                    sell_prices[key] = bp
                self._bear_stock_holding = None
            else:
                sell_reasons[key] = f"熊转牛-减仓农业银行为{best_name}补仓"
                if bp is not None:
                    sell_prices[key] = bp
                self._bear_stock_holding["weight"] = bear_cap
    ...
    def _check_bear_stock_buy(
        ...
        if (
            ...
            return
        if not self._is_bear_stock_signal_active(trade_date):
            return
        ...
        if (
            self._bear_stock_holding is None
            ...
            self._bear_stock_holding = {
                ...
                "buy_date": execution_date,
                "next_profit_target": self.BEAR_STOCK_INITIAL_PROFIT_TARGET,
                "last_action": "init_buy",
            }
            buy_prices[buy_key] = execution_price
            return

        current_weight = float(self._bear_stock_holding["weight"])
        if current_weight >= self.BEAR_STOCK_POSITION - self.BEAR_STOCK_MIN_WEIGHT:
            return

        add_weight = self.BEAR_STOCK_POSITION - current_weight
        old_buy_price = float(self._bear_stock_holding["buy_price"])
        total_cost = old_buy_price * current_weight + execution_price * add_weight
        new_weight = current_weight + add_weight
        self._bear_stock_holding["buy_price"] = total_cost / new_weight if new_weight > 0 else execution_price
        self._bear_stock_holding["weight"] = new_weight
        self._bear_stock_holding["buy_date"] = execution_date
        self._bear_stock_holding["next_profit_target"] = self.BEAR_STOCK_REBUY_PROFIT_TARGET
        self._bear_stock_holding["last_action"] = "rebuy"
        buy_prices[buy_key] = execution_price

    def _is_bear_stock_signal_active(self, trade_date: pd.Timestamp) -> bool:
        if not hasattr(self, "_bear_stock_factor_lookup") or not self._bear_stock_factor_lookup:
            return False
        date_n = pd.Timestamp(trade_date).normalize()
        bear_data = self._bear_stock_factor_lookup.get(date_n)
        if bear_data is None:
            return False
        try:
            kdj_j = float(bear_data["kdj_bfq"])
            close = float(bear_data["close"])
            ma30 = float(bear_data["ma_bfq_30"])
        except (TypeError, ValueError, KeyError):
            return False
        if pd.isna(kdj_j) or pd.isna(close) or pd.isna(ma30):
            return False
        return bool(kdj_j < self.BEAR_STOCK_KDJ_J_THRESHOLD and close < ma30)

    def _check_bear_stock_profit_sell(
        self,
        trade_date: pd.Timestamp,
        sell_reasons: dict[str, str],
        sell_prices: dict[str, float],
    ) -> None:
        """农业银行动态止盈：首仓盈利5%卖半；后续需补仓后再按整体盈利3%卖半。"""
        if (
        ...
        if pd.Timestamp(self._bear_stock_holding["buy_date"]) >= pd.Timestamp(trade_date):
            return

        current_price = self._get_bear_stock_price(trade_date)
        if current_price is None or current_price <= 0:
            return

        current_return = current_price / self._bear_stock_holding["buy_price"] - 1.0
        target = self._bear_stock_holding.get("next_profit_target")
        if target is None:
            return

        if current_return > target:
            self._bear_stock_holding["weight"] *= 0.5
            key = f"{self.BEAR_STOCK_CODE}|{trade_date.strftime('%Y-%m-%d')}"
            sell_reasons[key] = f"农业银行动态止盈-收益{current_return:.1%}>{target:.1%}卖出一半"
            sell_prices[key] = current_price
            self._bear_stock_holding["next_profit_target"] = None
            self._bear_stock_holding["last_action"] = "take_profit"
            self._bear_stock_last_reduce_signal_date = pd.Timestamp(trade_date)

            if self._bear_stock_holding["weight"] < self.BEAR_STOCK_MIN_WEIGHT:
                self._bear_stock_holding = None
    ...
    def _check_bear_stock_stop_loss(
        ...
        if len(recent) < 2 or not self._hit_stop_loss(recent):
            return False
        key = f"{self.BEAR_STOCK_CODE}|{trade_date.strftime('%Y-%m-%d')}"
        sell_reasons[key] = "止损-跌破大哥线及近期低点"
