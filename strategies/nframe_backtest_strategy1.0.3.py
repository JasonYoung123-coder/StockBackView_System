...
class Strategy(BaseStrategy):
    """Nframe 选股回测策略 v1.0.3 — 信号密度与质量择时版。

    在 v1.0 基础上新增"方案B: 信号密度与质量择时"：
    - 每日记录候选股数量、平均得分、5日/4日模式分布
    - 用滚动 N 日窗口统计信号密度均值与平均得分
    - 信号密度极低 → 市场无此类机会，暂停建仓
    - 信号密度极高 → 市场恐慌后假信号，降仓
    - 平均得分偏低 → 减少持仓数量
    - 回测结束后将每日信号记录导出到本地 Excel 文件
    """

    name = "Nframe选股回测策略v1.0.3信号密度择时"
    description = (
        "基于放量缩量回调模式选股（5日/4日），"
        "叠加信号密度与质量择时，回测后导出每日信号日志Excel。"
    )
    is_portfolio_strategy = True
    lookback_days = 120

    _SIGNAL_LOG_DIR = Path(__file__).resolve().parents[1] / "analysis"
    ...
    def __init__(
        ...
        timing_density_window: int = 20,
        timing_density_low: float = 0.5,
        timing_density_high: float = 10.0,
        timing_score_min: float = 3.0,
    ) -> None:
        ...
        self.timing_density_window = int(timing_density_window)
        self.timing_density_low = float(timing_density_low)
        self.timing_density_high = float(timing_density_high)
        self.timing_score_min = float(timing_score_min)

    def get_config_schema(self) -> dict:
        return {
            "title": "Nframe v1.0.3 信号密度择时参数",
            "fields": [
                {"name": "max_holdings", "label": "满仓最大持股数", "type": "number", "min": 1, "max": 10, "step": 1, "default": self.max_holdings},
                {"name": "timing_density_window", "label": "择时-信号密度滚动窗口(天)", "type": "number", "min": 5, "max": 60, "step": 1, "default": self.timing_density_window},
                {"name": "timing_density_low", "label": "择时-密度低于此值暂停建仓", "type": "number", "min": 0, "max": 5, "step": 0.1, "default": self.timing_density_low},
                {"name": "timing_density_high", "label": "择时-密度高于此值降仓", "type": "number", "min": 3, "max": 50, "step": 1, "default": self.timing_density_high},
                {"name": "timing_score_min", "label": "择时-平均得分低于此值降仓", "type": "number", "min": 1, "max": 8, "step": 0.5, "default": self.timing_score_min},
    ...
    def generate_portfolio_weights(
        ...
        daily_log: list[dict] = []
        win = self.timing_density_window
        ...
        for day_idx, trade_date in enumerate(trade_index):
            ...
            # ── 获取当日全部候选（不受 excluded 限制），用于统计信号密度 ──
            all_cands = self._find_candidates(td_np, daily_candidates, stock_cache, set())
            day_count = len(all_cands)
            day_avg_score = float(np.mean([c["score"] for c in all_cands])) if all_cands else 0.0
            cnt_5d = sum(1 for c in all_cands if c["pattern"] == "5日")
            cnt_4d = day_count - cnt_5d

            density_history.append(day_count)
            score_history.append(day_avg_score)

            # ══════ 0. 信号密度择时判定 ══════
            if len(density_history) >= win:
                rolling_density = float(np.mean(density_history[-win:]))
                rolling_score = float(np.mean(score_history[-win:]))
            else:
                rolling_density = float(np.mean(density_history))
                rolling_score = float(np.mean(score_history)) if score_history else 0.0

            if rolling_density < self.timing_density_low:
                effective_max = 0
                timing_regime = "暂停-信号稀疏"
            elif rolling_density > self.timing_density_high:
                effective_max = max(1, self.max_holdings // 2)
                timing_regime = "降仓-信号过密"
            elif rolling_score < self.timing_score_min and len(density_history) >= win:
                effective_max = max(1, self.max_holdings // 2)
                timing_regime = "降仓-质量偏低"
            else:
                effective_max = self.max_holdings
                timing_regime = "满仓"

            top = all_cands[0] if all_cands else None
            top_label = f"{top['ts_code']}({top['score']}分/{top['pattern']})" if top else ""
            ...
            daily_log.append({
                ...
                "4日模式数": cnt_4d,
                "当日最高分股票": top_label,
                "当日平均得分": round(day_avg_score, 2),
                "滚动密度均值": round(rolling_density, 2),
                "滚动得分均值": round(rolling_score, 2),
                "择时状态": timing_regime,
                "有效最大持仓": effective_max,
                "实际持仓数": len(holdings),
            })
            ...
            if available > 0:
                ...
                cands_filtered = [c for c in all_cands if c["ts_code"] not in excluded]
                for cand in cands_filtered[:available]:
        ...
        # ── 导出每日信号记录到 Excel ──
        self._export_signal_log(daily_log)
    ...
    def _export_signal_log(self, daily_log: list[dict]) -> None:
        if not daily_log:
            return
        out_dir = self._SIGNAL_LOG_DIR
        out_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = out_dir / f"nframe_signal_density_log_{ts}.xlsx"
        df = pd.DataFrame(daily_log)
        try:
            df.to_excel(path, index=False, engine="openpyxl")
            self._warnings.append(f"信号密度日志已保存: {path}")
        except Exception as exc:
            self._warnings.append(f"信号密度日志保存失败: {exc}")
