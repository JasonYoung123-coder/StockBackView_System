...
class Strategy(BaseStrategy):
    ...
    def generate_portfolio_weights(
        ...
        for idx, trade_date in enumerate(trade_index, start=1):
            ...
            if oamv_regime == "死叉" and holdings:
                self._force_sell_all_on_dead_cross(
    ...
    DEAD_CROSS_KEEP_MIN_RETURN = 0.15

    def _force_sell_all_on_dead_cross(
        self,
        trade_date: pd.Timestamp,
        holdings: dict[str, dict],
        sell_reasons: dict[str, str],
        ...
        """活跃市值死叉：浮盈>=15%且未触发止损止盈的股票继续持有，其余次日开盘清仓。"""
        for ts_code in list(holdings.keys()):
            holding = holdings[ts_code]
            ...
            if holding["buy_price"] > 0:
                ...
                if unrealized_return >= self.DEAD_CROSS_KEEP_MIN_RETURN:
                    should_sell, _, _ = self._should_sell(ts_code, trade_date, holding)
                    if not should_sell:
                        continue
            ...
            sell_reasons[signal_key] = "活跃市值死叉-强制清仓"
