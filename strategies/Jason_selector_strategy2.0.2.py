...
class Strategy(BaseStrategy):
    ...
    def _precompute_indicators(self, frame: pd.DataFrame) -> None:
        ...
        # ── 近20日最大成交量日是否收阳 (close >= open) ──
        def _vol_peak_bullish(g):
            c, o, v = g["close"].values, g["open"].values, g["vol"].values
            n = len(v)
            res = np.ones(n, dtype=bool)
            for i in range(n):
                start = max(0, i - 19)
                j = start + int(np.argmax(v[start:i + 1]))
                res[i] = c[j] >= o[j]
            return pd.Series(res, index=g.index)

        frame["max_vol_20d_is_up"] = frame.groupby(
            ts_col, sort=False, group_keys=False,
        ).apply(_vol_peak_bullish)
    ...
    def _rank_candidates(
        ...
        # 近20日最大成交量日必须收阳，否则踢出候选
        if "max_vol_20d_is_up" in c.columns:
            c = c.loc[c["max_vol_20d_is_up"]].copy()
            if c.empty:
                return pd.DataFrame()
    ...
    def _try_replace_holdings(
        ...
        use_relaxed = False
        if candidates.empty:
            # 无 7 分候选时，对非 7 分持仓按得分从高到低用最优候选替换
            replaceable = [
                (code, ret) for code, ret in replaceable
                if float(holdings[code].get("score", 0)) < self.buy_score_threshold
            ]
            if not replaceable:
                return replaced
            candidates = self._rank_candidates(prepared, trade_date, holdings, min_score=0)
            if candidates.empty:
                return replaced
            use_relaxed = True
        ...
        for old_code, _ in replaceable:
            ...
            reason = "调仓-按得分替换非满分持仓" if use_relaxed else "调仓-替换为更优股票"
            sell_reasons[f"{old_code}|{trade_date.strftime('%Y-%m-%d')}"] = reason
    ...
    def _ensure_chips(
        ...
        # Tushare cyq_chips API 有行数限制且从区间末尾开始返回，
        # 用紧凑窗口（目标日期前后各 2 个交易日）确保目标日期不被截断。
    ...
    def _load_oamv_regime(self, trade_index: pd.DatetimeIndex) -> pd.Series:
        ...
        # 回溯回测开始前的所有 OAMV 事件，推导正确的初始状态。
        # 避免回测恰好从金叉区间中段开始时被误判为死叉。
        first_date = normalized_dates[0] if normalized_dates else None
        if first_date is not None:
            pre_events = frame.loc[frame["trade_date"] < first_date].sort_values("trade_date")
            last_big_rise_date: pd.Timestamp | None = None
            for _, pre_row in pre_events.iterrows():
                evt = pre_row["status"]
                if evt == "金叉":
                    current_state = "金叉"
                    strict_state = "金叉"
                    last_big_rise_date = None
                elif evt == "死叉":
                    current_state = "死叉"
                    strict_state = "死叉"
                    last_big_rise_date = None
                    ...
                    if strict_state == "金叉":
                        ...
                        current_state = "大涨"
                        last_big_rise_date = pre_row["trade_date"]
            if current_state == "大涨" and last_big_rise_date is not None:
                bdays = pd.bdate_range(last_big_rise_date, first_date)
                bdays_after = len(bdays) - 1
                if bdays_after >= self.oamv_big_rise_window:
                    current_state = "死叉"
                else:
                    pending_dead_idx = self.oamv_big_rise_window - 1 - bdays_after
