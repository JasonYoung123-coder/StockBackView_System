...
class Strategy(BaseStrategy):
    ...
    def generate_portfolio_weights(
        ...
        for idx, trade_date in enumerate(trade_index, start=1):
            ...
            if oamv_regime == "死叉":
                if self._is_quick_dead_cross(trade_date) and holdings:
                    cleared = self._clear_all_holdings_on_quick_dead_cross(
    ...
    def _is_quick_dead_cross(self, trade_date: pd.Timestamp) -> bool:
        if not hasattr(self, "_oamv_quick_dead_cross_dates"):
            return False
        return pd.Timestamp(trade_date).normalize() in self._oamv_quick_dead_cross_dates

    def _clear_all_holdings_on_quick_dead_cross(
        self,
        trade_date: pd.Timestamp,
        holdings: dict[str, dict],
        sell_reasons: dict[str, str],
        ...
        for ts_code in list(holdings.keys()):
            ...
            sell_reasons[signal_key] = f"快速死叉清仓-金叉后≤{self.short_golden_cross_max_days}日即死叉次日开盘全清"
    ...
    def _load_oamv_regime(self, trade_index: pd.DatetimeIndex) -> pd.Series:
        ...
        regime_series, quick_dead_dates = self._build_regime_series(event_map, trade_index)
        self._oamv_quick_dead_cross_dates = quick_dead_dates
    ...
    def _build_regime_series(
        ...
        同时检测"快速死叉"：金叉后 <= short_golden_cross_max_days 个交易日
        即出现死叉，返回这些死叉日期的集合。
        """
        regimes: list[str] = []
        current_state = "死叉"
        ...
        last_golden_cross_idx: int | None = None
        quick_dead_cross_dates: set[pd.Timestamp] = set()
        ...
        for idx, trade_date in enumerate(normalized_dates):
            ...
            if event_status == "金叉":
                ...
                last_golden_cross_idx = idx
            elif event_status == "死叉":
                if (
                    last_golden_cross_idx is not None
                    and idx - last_golden_cross_idx <= self.short_golden_cross_max_days
                ):
                    quick_dead_cross_dates.add(trade_date)
                current_state = "死叉"
                in_golden_cross = False
                pending_dead_idx = None
        ...
        return pd.Series(regimes, index=trade_index, dtype="object"), quick_dead_cross_dates
